---
title: "Climate Change Science"
subtitle: "Climate activists, climate records, and ???"
author: "EA30 Students, Pomona College"
date: '`r format(Sys.Date(), "%B %d, %Y")`'
output:
  tufte::tufte_html: default
  tufte::tufte_handout:
    citation_package: natbib
    latex_engine: xelatex
  tufte::tufte_book:
    citation_package: natbib
    latex_engine: xelatex
bibliography: skeleton.bib
link-citations: yes
---
Updated: "Last compiled on `r format(Sys.time(), '%d %B, %Y')`"

```{r setup, include=FALSE}
library(tufte)
# invalidate cache when the tufte version changes
knitr::opts_chunk$set(tidy = FALSE, cache.extra = packageVersion('tufte'))
options(htmltools.dir.version = FALSE)
```
# Introduction

## Climate Activists and Defining Regional Narratives

Climate change science is complex -- but some relatively simple conclusions can be made: 1) humans are adding more CO~2~ to the atmosphere than the Earth can assimilate; 2) CO~2~ acts as a greenhouse gas and can regulate the Earth's temperature; 3) the anthropogenic sources are warming the surface of the Earth; 4) there is growing evidence that that anthropogenic CO2 emmissions are warming the Earth but have differential impacts regionally. 

What are these regional impacts? How are regional stakeholders dealing with these impacts?

As you might guess, every region in the US and beyond have very different narratives and impacts -- as as such, actvitists will  the issues of climate change from different perspectives. 

For our 2020 project, we document how various activists confront climate data and respond in terms of public policy. 

 - Activists might have an important role in defining and framing climate issues at regional levels.

```{r echo=FALSE, fig.margin = TRUE, out.width='100%', fig.cap='Tribute photo for Ken Saro Wiwa anniversary. Reflection in polluted river of local activist who follows in Ken"s footsteps, Kegbaradere community Luka Tomac/Friends of the Earth International. Taken 5 November 2015. Some rights reserved. creativecommons license 2.0'}
knitr::include_graphics('F2020/Images/KenSara-Wiwa.jpg')
```

## Weather Records and Climate Trends

Weather records date back to the early 1600s. However, a robust, spatially distributed set of records did begin until the late 1800s and only after 1950, are the records beginnning to cover the majority of the Earth's land surface. 

As Bruno Latour has noted the process to modernize is both uneven and inconsistent [@latour2012we; @pachauri2015ipcc]. And as we look to use weather stations, which had been established answer specific local questions, e.g. when will there be enough water to plant rice or warm enough to sow corn. But as we try to evaluate how the ``average weather" or climate might be changing we learn about how complex it is to use these weather stations. 

# Projects for 2020

## Hydrologic and Farming Impacts

Climate change will not only change temperatures, but also rainfall patterns. In rural communities, which are often rely on agriculture, changes in rainfall and streamflow can have imporant impact on relative properity of the community. For example, [Burns (2020)](F2020/Burns.html) evaluated how climate and rainfall may impact rural areas of north eastern Illinois. 



The ecological impacts on For example, [Duggleby (2020)](F2020/Duggleby.html)...

## New England

[Hoffsten (2020)](F2020/Hoffsten.html) explores the impact of climate change on Maine's environment and social and economic infrastructure and advocates for the orientation of Indigenous narratives within climate conversations.

Hook...For example, [LeBlanc (2020)](F2020/LeBlanc.html)...

## Urban Heat

For example, Chicago should expect to have an increase in temperture, which is further exaxperated by the heat island effectd [(McGrath 2020)](F2020/McGrath.html).


## Theme 2... tbd

Hook...For example, [Marsh (2020)](F2020/Marsh.html)...

Hook...For example, [Nguyen (2020)](F2020/Nguyen.html)...



Hook...For example, [Rubin (2020)](F2020/Rubin.html)...

Hook...For example, [Waring (2020)](F2020/Waring.html)...


Hook...For example, [Williams (2020)](F2020/Williams.html)...

Added students...

Europeans have also reason to pay attention, [Bisaga (2020)](F2020/Bisaga.html) documents potential climate impacts in Poland.


[LeBlanc (2020)](F2020/LeBlanc.html)

[Marsh (2020)](F2020/Marsh.html)

[McGrath (2020)](F2020/McGrath.html)

[Meilenger (2020)](F2020/Meilenger.html)

[Nguyen (2020)](F2020/Nguyen.html)

## Urban Health and Equity

Climate change has specific and differential impacts on health outcomes and in many case, fall disporportionately on vulnerable populations. [Plancarte (2020)](F2020/Plancarte.html) documents how climate change has impacted San Diego, CA with increasing heat. [Greene 2020)](F2020/Greene.html) follows a similar thread for Washington D.C.  In contrast, [Tahbaz (2020)](F2020/Tahbaz.html) evaluates the non-intuitive impact of snow events in NYC and it's social justice implications. 

## Economic Impacts on Agriculture

How will climate change affect prosperous agricultural regions in the country? [Rubin (2020)](F2020/Rubin.html) explores the the ecological and economical threats of anthropogenic climate change to the mitten state, while [Manning (2020)](F2020/Manning.html) evaluates climate change in a small town in Idaho. 

## The South

[Swiere (2020)](F2020/Swiere.html) documents the impact on climate in Texas and why we should care... :-)


[Waring (2020)](F2020/Waring.html)

[Williams (2020)](F2020/Wiliams.html)

>>>>>>> 538073b0cda7f615009b8e70d4f84d82070a3b18

# Previous EA30 Projects

[Climate Records, Alpine Environments, and Coastal Impacts (Fall 2019)](F2019/index_2019F.html)

[Attribution Science (Spring 2019)](S2019/index_2019S.html)

[Climate Change and Fires -- Is this a red herring or the alarm alarmists have been warning about? (Fall 2918)](F2018/index_2018F.html)

[Why Climate Change Will Affect You Personally? (Fall 2017)](F2017/index_2017F.html)

[Climate Change and Logical Fallacies Spring 2016)](S2017/index_2017S.html)
