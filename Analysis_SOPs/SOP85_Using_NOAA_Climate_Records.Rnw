...
We will be using the program R, to run our data analysis. To access the program R we will be using the platform Rstudio which is available on the cloud to students. Since we will be using cloud based computing it will require us to know a little bit about how to access the resources online. Thus, it might be useful to read SOP_6 on the basics of working with Rstudio and Github: You can click on the following link below to download the document: \\
\\
