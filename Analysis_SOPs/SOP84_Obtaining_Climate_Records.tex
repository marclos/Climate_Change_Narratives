...
\begin{description}
  ...
  \item[Select the Search Tool] additionally, you can use the mapping tool to evaluate which areas of the globe have available datasets but if you have a particular area in mind it might be easiest to search for that one first using the search tool
  \item[Specify Parameters]
    ...
    \item in \textbf{Select Data Range} change the range to the the earliest date available (so that you will be able to see how far back the data) on the list to the present. Although your site might not have these records, it's easier to set the greatest range upfront.
    \end{itemize}
