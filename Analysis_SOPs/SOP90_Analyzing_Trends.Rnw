...
Thailand <- CSV_import("/home/CAMPUS/mwl04747/github/Climate_Change_Narratives/Data/Thailand_GHCND:TH000048456.csv")
@
...
<<echo = F, fig = 'true' >>=
par(las=1)
plot(Thailand$NewDate, Thailand$TMAX, ylab="degrees C", xlab="Date", pch=20, main="TMAX, \n Bankok, Thailand" )
@
\end{figure}
...
<<echo = F, fig = 'true' >>=
par(las=1)
plot(Thailand$NewDate, Thailand$TMAX, type="l", ylab="degrees", xlab="Date", main="Temperature Changes, \n Mauna Loa, HI" )
...
plot(TMAX~YEAR, data=MonthlyMean[MonthlyMean$Month=="12",], ty='l', las=1) 
...
3. In this case, we get a slope, \Sexpr{coef(Dec.lm)[2]} degress/year and a probability of \Sexpr{round(summary(Dec.lm)$coefficients[2,4],7)} and an r-sqaured of \Sexpr{round(summary(Dec.lm)$r.squared, 3)}. Cool! As we might expect, the a small amount of the variance is explained by the ``Month.'' Many things predict temerpature, that year is one, is quite problematic.
