...
```{r}
BarrowAlaska$SNWD[BarrowAlaska$PRCP==-9999] <- NA
...
```{r} 
plot(PRCP~DATE, import)
```
```{r, include=FALSE}
plot(PRCP~DATE, import[import$DATE<19131231,], ty='l')
```
```{r, include=FALSE}
strDates <- as.character(import$DATE)
head(strDates)
...
```
```{r, include=FALSE}
plot(PRCP~NewDate, import[import$DATE<19130102,], ty='l')
```

```{r}
BarrowAlaska.lm <- lm(PRCP~NewDate, data=BarrowAlaska)
summary(BarrowAlaska.lm)
plot(PRCP~NewDate, BarrowAlaska, ty='l')
abline(coef(BarrowAlaska.lm), col='red')
```

```{r}
MonthlyMeanPRCP = aggregate(PRCP ~ Month + Year, BarrowAlaska, mean)
...
```{r}
MonthlyMean = aggregate(PRCP ~ Month + Year, BarrowAlaska, mean)

MonthlyMean$YEAR = as.numeric(MonthlyMean$Year)
MonthlyMean$MONTH = as.numeric(MonthlyMean$Month)

MonthlySD = aggregate(PRCP ~ Month + Year, BarrowAlaska, sd)
...
### Precipitation Trends in December, January, February

### Snowfall Trends in December, January, February


### Snow Depth Trends in December, January, February

```{r December2}
plot(MonthlyMean$SNWD[MonthlyMean$Month=="12"], ty='l')
plot(SNWD~YEAR, data=MonthlyMean[MonthlyMean$Month=="12",], ty='l') 
December.lm <- lm(SNWD~YEAR, data=MonthlyMean[MonthlyMean$Month=="12",])
summary(December.lm)
...
```{r January3}
plot(MonthlyMean$SNWD[MonthlyMean$Month=="01"], ty='l')
plot(SNWD~YEAR, data=MonthlyMean[MonthlyMean$Month=="01",], ty='l') 
January.lm <- lm(SNWD~YEAR, data=MonthlyMean[MonthlyMean$Month=="01",])
summary(January.lm)
...
```{r February3}
plot(MonthlyMean$SNWD[MonthlyMean$Month=="02"], ty='l')
plot(SNWD~YEAR, data=MonthlyMean[MonthlyMean$Month=="02",], ty='l') 
February.lm <- lm(SNWD~YEAR, data=MonthlyMean[MonthlyMean$Month=="02",])
summary(February.lm)
