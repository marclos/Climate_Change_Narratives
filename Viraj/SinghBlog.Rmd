...
Monroe County, Florida is a coastal community where tourism, coastal infrastructure, and freshwater access are fundamental to economic and social well being. The IPCC asserts that environmental hazards stemming from rising temperature such as increased flood risk, sea level rise, etc threaten the economic and social well being of Southern Florida. In an attempt to understand these claims, I investigated and analyzed the local temperature data of Monroe County.

**Methods:**

I downloaded data freely available from NOAA (National Oceanic and Atmospheric Administration) via a weather station in Monroe County (Everglades), Florida. I used rstudio to summarize and evaluate the data. I then analyzed maximum and minimum temperatures (F) by month since 1924 to present to account for daily changes in temperature (night vs day). 

I am determining a trend between time and temperature using the null hypothesis method, a standard scientific convention. The null hypothesis states that there is no relationship between temperature and time. Therefore, if the probability value calculated is less than .05, the null hypothesis is considered violated and a relationship can be assumed.
...
plot(TMAX~YEAR, data=MonthlyMean[MonthlyMean$Month=="05",], ty='p',pch=20) 
...
```{r, echo=F}
slopePvalue <- matrix(c(-3.86,"<.01",-3.62,.01,-2.02,.09,-3.80,"<.01",-1.92,.02,-1.70,.049,-1.36,.02,-1.91,"<.01",-1.38,.02,.31,.69,-.1,.92,-.29,.83),ncol=2,byrow=TRUE)
...
plot(TMIN~YEAR, data=MonthlyMeanTMIN[MonthlyMeanTMIN$Month=="05",], ty='p', pch=20) 
