...
library(readxl)
Burns_DP_Alternate <- read_excel("Data/FA20/Burns_DP_Alternate.xlsx", 
...
```{r}
plot(sqrt(PeakFlow)~Date, data=Burns_DP_Alternate)
PeakFlow.lm <- lm(sqrt(PeakFlow)~Date, data=Burns_DP_Alternate)
summary(PeakFlow.lm)
coef(PeakFlow.lm)
