...
To test this trend out, all of the data was downloaded from NOAA (National Oceanic and Atmospheric Administration) from a weather station in Santa Monica, California. The RStudio software was used to summarize, evaluate, and graph the data. When processing the data, the obviuos outliers were delted (ie. incorrectly inputted high/low temperatures) and a linear model was then created, along with a trendline. Then maximum temperatures (celcius) were analyzed from 1924 to 2017 to account for yearly and seasonal fluctuations in temperature. The months of July and December were also graphed and analyzed. This was then used to see if each month fits the general trend that the yearly temperature shows.  

Now it will be determined if there is a trend between time and temperature using the null hypothesis. The null  hypothesis predicts that there is no observed relationship between variables.
In this case, the null hypothesis predicts that there is no relationship between the maximum tmperature of Santa Monica and time. A rejection of the null hypothesis supports the alternative hypothesis that there is a relationship between maximum temperature of Santa Monica and time. 
...
##What climate change means for santa monica
