...
## As seen below, the maximum temperatures in September, from 1940 to 2020, display a rising trend. Having lived in Chicago all my life, fall tends to begin early, with temperatures dropping around September and October. However, as the graph below shows, autumn has been getting warmer and warmer. 
...
## Fixing the Format of Month and Year as numeric
#MonthlyPRCPMean$YEAR = as.numeric(MonthlyPRCPMean$Year)
#MonthlyPRCPNMean$MONTH = as.numeric(MonthlyPRCPMean$Month)
...
#PRCPresult <- rbind(PRCPresult, 
                    #  cbind(Months[i], 
                            #round(coef(MonthPrcp_lm)[2], 3), # go in to the coef(MonthMin_lm) object and take the 2nd element and round the number to 4 digits
                           # round(summary(MonthPrcp_lm)$coefficients[2,4], 3), # What does this column correspond to? Try copying just round(summary(MonthMin_lm)$coefficients[2,4], 4) after running an example MonthMin_lm <- lm(...) command in the console and see what it spits out
                           # round(summary(MonthPrcp_lm)$r.squared, 2))) # create a column for the Rsquared value
...
## Means by Month (not in graph form)
```{r index}
