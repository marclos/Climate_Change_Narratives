...
attach(climate_data)
par(mfrow=c(2,2))
layout(matrix(c(1, 2, 3, 4), 2, 2, byrow = TRUE), widths=c(6,6), heights=c(6,6))
plot(TMIN~YEAR, data = MonthlyTMINMean[MonthlyTMINMean$Month=="08",], ty="l", xlim=c(1891, 2020), main="Average Mininum Temperature (August)", ylab="Min Temperature (C)", xlab="Year", las=1)
AugustTMIN.lm <- lm(TMIN~YEAR, data = MonthlyTMINMean[MonthlyTMINMean$Month=="08",])
abline(coef(AugustTMIN.lm), col="red")
summary(AugustTMIN.lm)
plot(TMIN~YEAR, data = MonthlyTMINMean[MonthlyTMINMean$Month=="05",], ty="l", xlim=c(1891, 2020), main="Average Minimum Temperature (May)", ylab="Min Temperature (C)", xlab="Year", las=1)
MayTMIN.lm <- lm(TMIN~YEAR, data = MonthlyTMINMean[MonthlyTMINMean$Month=="05",])
abline(coef(MayTMIN.lm), col="purple")
summary(MayTMIN.lm)
plot(TMAX~YEAR, data=MonthlyTMAXMean[MonthlyTMAXMean$Month=="08",],
ty='l', xlim=c(1900, 2020), main="Average Maximum Temperature (August)", ylab="Max Temperature (C)", xlab="Year", las=1)
