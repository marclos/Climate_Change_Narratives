...
```{r Precipitation}
plot(MonthlyPRCPMean$PRCP~MonthlyPRCPMean$Month)
```

```{r Precipitation selecting for one month}
climate_data$Month = format(as.Date(climate_data$NewDate), format = "%m")
climate_data$Year = format(climate_data$NewDate, format="%Y")
...
Months = c("January", "February", "March", "April",
    "May", "June", "July", "August", "September", "October",
    "November", "December")
plot(PRCP~Year, data=MonthlyPRCPMean[MonthlyPCRPMean$Month=="05",], ty='l', xlim=c(1950, 2020))
...
```{r PRCP selecting for one month MAY}
climate_data$Month = format(as.Date(climate_data$NewDate), format = "%m")
climate_data$Year = format(climate_data$NewDate, format="%Y")
...
Months = c("January", "February", "March", "April",
    "May", "June", "July", "August", "September", "October",
    "November", "December")
plot(PRCP~Year, data=MonthlyPRCPMean[MonthlyPRCPMean$Month=="05",], ty='l', xlim=c(1950, 2020))
May.lm <- lm(PRCP~YEAR, data=MonthlyPRCPMean[MonthlyPRCPMean$Month=="05",]) 
summary(May.lm)
abline(coef(May.lm), col="red")
```

```{r Precipitation Month December}
str(MonthlyPRCPMean)
plot(MonthlyPRCPMean$PRCP, ty='l')
Months = c("January", "February", "March", "April",
    "May", "June", "July", "August", "September", "October",
    "November", "December")
plot(PRCP~Year, data=MonthlyPRCPMean[MonthlyPRCPMean$Month=="12",], ty='l', xlim=c(1950, 2020))
December.lm <- lm(PRCP~YEAR, data=MonthlyPRCPMean[MonthlyPRCPMean$Month=="12",]) 
summary(December.lm)
