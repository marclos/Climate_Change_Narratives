...
This can also be said with certainty as the model has a p value of 4.661e-06 which is also significantly less that 0.05. Note: The slope of this graph looks much steeper because the range of temperatures is far less than over the full year. Analysis of the other months of the year was done and all showed the same trends, although some less significantly. The only month that did not have a significant p value was January, whose p value was 0.391.

```{r, echo=FALSE}
plot(TMAX~YEAR, data=MonthlyTMAXMean[MonthlyTMAXMean$Month=="05",], ty='p', pch=20, main = "Average Maximum Temperatures in May")
...
I also have done the same analyses of the minimum themperatures, showing similar trends to the maximum. Here is the daily minimum temperatures and a linear model, with a p-value of 2e-16, so the model is very accurate. 
...
##abline(coef(climate.lm), col="blue")
plot(TMIN~YEAR, data=MonthlyTMINMean[MonthlyTMINMean$Month=="05",], ty='p', pch=20, main = "Average Minimum Temperature Data in May")
