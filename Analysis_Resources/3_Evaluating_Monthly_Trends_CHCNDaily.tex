...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(predvars, data, env): object 'YEAR' not found}}\end{kframe}
\end{knitrout}
\end{figure}
...
Although for today, we are not going to try to interpret these figures, but you should notice there is a ton of unaccounted structure, i.e. variance, in the model. This is due, in part, to a violation of independence; these data are serially correlated and the model does not account for that and is inappropriate because of this. It also appears that a straight-line model does not fit well and a curvilinear should be investigated.

A properly specified model is shown in\ldots\footnote{Coming soon} 
...
\section{Is there a direct relationship between CO2 and temperature?}

\subsection{Year is not a mechanism}

Although we have looked at temperture change over time, we don't have an actual mechanism as an explanatory factor. In fact, the only thing time is useful for is to explain age. But ailments associated with aging is not caused directly, but indirectly with age. Thus, the addage of correlatina is not causation. 

So, can we directly test the role of CO2 on climate?  Let's find out! 



\begin{figure}
\label{fig:carbondioxide}
\caption{Carbon Dioxide and TMIN.}
%\setkeys{Gin}{width=0.75\textwidth} % LaTeX code to read the graphic file in at 75% of its original size
% R code chunk that produces a graphic
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
...
\hlkwd{plot}\hlstd{(TMIN} \hlopt{~} \hlstd{average,} \hlkwc{data}\hlstd{=MonthlyTMINMean,} \hlkwc{ty}\hlstd{=}\hlstr{'p'}\hlstd{,} \hlkwc{pch}\hlstd{=}\hlnum{20}\hlstd{,} \hlkwc{cex}\hlstd{=}\hlnum{.4}\hlstd{,} \hlkwc{col}\hlstd{=}\hlstr{'gray'}\hlstd{,} \hlkwc{xlab}\hlstd{=}\hlstr{"[CO2]"}\hlstd{,} \hlkwc{ylab}\hlstd{=}\hlstr{"Temperature C"}\hlstd{)}

\hlstd{CO2.lm}\hlkwb{<-} \hlkwd{lm}\hlstd{(TMIN} \hlopt{~} \hlstd{average,} \hlkwc{data}\hlstd{=MonthlyTMINMean)}
\hlkwd{summary}\hlstd{(CO2.lm)}
\end{alltt}
\begin{verbatim}
## 
## Call:
## lm(formula = TMIN ~ average, data = MonthlyTMINMean)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
## -7.7951 -0.8450  0.4486  1.3057  4.1161 
## 
## Coefficients:
##              Estimate Std. Error t value Pr(>|t|)
## (Intercept) 16.421134   0.950709  17.273   <2e-16
## average      0.022660   0.002679   8.459   <2e-16
##                
## (Intercept) ***
## average     ***
## ---
...
## 0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 1.833 on 641 degrees of freedom
## Multiple R-squared:  0.1004,	Adjusted R-squared:  0.09902 
## F-statistic: 71.56 on 1 and 641 DF,  p-value: < 2.2e-16
\end{verbatim}
\begin{alltt}
\hlkwd{abline}\hlstd{(}\hlkwd{coef}\hlstd{(CO2.lm))}
\end{alltt}
\end{kframe}
...
I can already see problems with this figure... 1) the co2 are serially correlated which shows up as odd diagonal points. 2) Doesn't really tell us the that earth is warming through time, however, the mechanism is more direct!
