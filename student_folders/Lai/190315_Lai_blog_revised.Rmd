...
##Summarize figure 5 by saying high variability rather than showing it.

and daily precipitation over time (Figure 6) I created for D-1 station below. The height of the 2013 floods is clearly visible as the tallest spike on the right side of Figure 6. I purposely omit the graphs of the other stations to prevent cluttering up the results section.

##EDIT PRCP TO MONTHLY AVERAGES! CONSIDER REMOVING DAILY TMAX GRAPH SINCE IT COULD BE CONCISELY SUMMARIZED IN ONE SENTENCE W/O NEED OF A GRAPH (HIGH VARIABILITY )  
```{r D1 daily TMAX & PRCP graphs, echo=FALSE}
plot(airtemp_max~date, D1_daily_temp, main="Figure 5. Daily Max Temp at D-1 Station", xlab = "Date (Year)", ylab = "Daily Max Temperature (°C)", ty='l', las=1)
abline(lm(airtemp_max ~ date, data=D1_daily_temp), col="red")
plot(pppt_tot~date, main="Figure 6. Daily Precipitation at D-1 Station", xlab = "Date (Year)", ylab = "Daily Precipitation Total", D1_daily_precip, ty='l', las=1)
abline(lm(pppt_tot ~ date, data=D1_daily_precip), col="red")
```
##EDIT GRAPHS!
