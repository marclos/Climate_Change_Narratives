...
``` {r min_and_max_tables, echo=FALSE, results='asis'}
library(xtable)
...
```
``` {r table_print, echo=FALSE, results='asis'}
...
head(PRCP)
plot(PRCP ~ Date, data=PRCP)
names(PRCP)=c("Month", "Year", "PRCP", "x", "Numeric.date")  
PRCP_mean = mean(PRCP$PRCP)
#plot(PRCP~Year, data=PRCP)
#abline(h=PRCP_mean, col="blue")
#need to add year to the date plot, push and leave note ** 
```
```{r PRCP.Sept.December, echo=FALSE}
plot(PRCP~Numeric.date, data=PRCP[PRCP$Month=="09",],
ty='l', xlim=c(171.8,177), xlab="Year", ylab="Sum of yearly precipitation (in)", main=
  "Total September precipitation 1955-2015")
Sept.lm <- lm(PRCP~Numeric.date, data=PRCP[PRCP$Month=="09",])
summary(Sept.lm)

plot(PRCP~Numeric.date, data=PRCP[PRCP$Month=="10",],
ty='l', xlim=c(172.9,177.9), xlab="Year", ylab="Sum of yearly precipitation (in)", main=
  "Total October precipitation 1955-2015")
Oct.lm <- lm(PRCP~Numeric.date, data=PRCP[PRCP$Month=="10",])
summary(Oct.lm)

plot(PRCP~Numeric.date, data=PRCP[PRCP$Month=="11",],
ty='l', xlim=c(173.9,178.8), xlab="Year", ylab="Sum of yearly precipitation (in)", main=
  "Total November precipitation 1955-2015")
Nov.lm <- lm(PRCP~Numeric.date, data=PRCP[PRCP$Month=="11",])
summary(Nov.lm)

plot(PRCP~Numeric.date, data=PRCP[PRCP$Month=="12",],
ty='l', xlim=c(174.9,179.9), xlab="Year", ylab="Sum of yearly precipitation (in)", main=
  "Total December precipitation 1955-2015")
Dec.lm <- lm(PRCP~Numeric.date, data=PRCP[PRCP$Month=="12",])
summary(Dec.lm)
