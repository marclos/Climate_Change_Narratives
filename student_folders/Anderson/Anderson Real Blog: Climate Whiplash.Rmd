...
This graph also shows monthly average temperature, but this time, it is showcasing the minimums. Once again we can reject the null hypothesis with a p-value of 0.001. Comparing the trend to the TMAX graph, it is easy to see the while TMIN is still increasing, it is doing so far less rapidly than TMAX, leading to an icreasing gap between the daily TMAX and TMIN. This is the first indication of climate whiplash; as gaps become more pronounced and varied over time, there will be a large effect on precipitation consistency, therefore kickstarting whiplash climate patterns.^6^
