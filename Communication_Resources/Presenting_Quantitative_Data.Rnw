...
<<fig.cap="Mean monthly maximum and minimum tempetures (C) ((Bangkok, Thailand, 1940-2018))">>=
par(mfrow=c(1,2))
...
<<fig.cap="Mean monthly maximum (Panel A) and minimum (Panel B) tempetures (C) ((Bangkok, Thailand, 1940-2018)).", fig.height=3.5, fig.width=6>>=
par(mfrow=c(1,2))
...
grades1 = c(28.5, 36.5, 25, 31.5, 26, 22, 22.5,
           2, 0.1, 22.5, 31.5, 6, 28, 34, 0.1)
grades2 = c(38.1, 39.9, 44.8, 34.4, NA, 28.4, NA,
            24.8, 17.5, 34.8, 37.9, 14.5, 37, NA, 16.6)

grades3 = c(42.6, NA, NA, 42.8, 43.8, NA, NA,
            34.8, NA, NA, NA, NA, NA, NA, NA)

grades4 = c(NA, NA, NA, 44.3, 46.5, NA, NA,
            NA, NA, NA, NA, NA, NA, NA, NA)

grades5 = c(NA, NA, NA, NA, 46.5, NA, NA,
            NA, NA, NA, NA, NA, NA, NA, NA)

grades6 = c(NA, NA, NA, NA, 48.5, NA, NA,
            NA, NA, NA, NA, NA, NA, NA, NA)
...
grades1_tab = table(cut(grades1, breaks = seq(0,50,10)))
grades2_tab = table(cut(grades2, breaks = seq(0,50,10)))
grades3_tab = table(cut(grades3, breaks = seq(0,50,10)))
grades4_tab = table(cut(grades4, breaks = seq(0,50,10)))
grades5_tab = table(cut(grades5, breaks = seq(0,50,10)))
grades6_tab = table(cut(grades6, breaks = seq(0,50,10)))
...
\bigskip
BTW: The mean was for the first draft was \Sexpr{round(mean(grades1, na.rm=T), 1)} and the second draft was \Sexpr{round(mean(grades2, na.rm=T), 1)}. Based on a paired t-test, the difference between the two grades is significant. 
...
\bigskip
<<scoreshist, fig.cap="Histogram of Blog Scores", fig.height=2.5, fig.width=6 >>=
par(mfrow=c(1,4), las=1)
hist(grades1, main="1st Draft", xlab="Score", 
     xlim=c(0,50), col="green")
hist(grades2, main="2nd Draft", xlab="Score", 
     xlim=c(0,50), col="blue")
hist(grades3, main="3rd Draft", xlab="Score", 
     xlim=c(0,50), col="darkgreen")
hist(grades4, main="4th Draft", xlab="Score", 
     xlim=c(0,50), col="purple")
@
...
<<scoresdensity, fig.cap="Probability Density for Scores", fig.height=5, fig.width=8>>=
...
colors <- c("red", "blue", "darkgreen", "gold", "violet", "black")
labels <- c("Draft 1", "Draft 2", "Draft 3", "Draft 4", "Draft 5", "Final")

plot(x, hx1, type="l", lty=1, lwd=3, xlab="Score",
  ylab="Density", main="Distribution of Scores", las=1, 
  ylim=c(0,.15), col=colors[1])
  lines(x, hx2, lwd=3, col=colors[2])
  lines(x, hx3, lwd=3, col=colors[3])
  lines(x, hx4, lwd=3, col=colors[4])
ttest = t.test(grades1, grades2, paired=T)

#text(ttest$estimate[1], max(hx1), "Draft 1 Mean", pos=3)
#text(ttest$estimate[2], max(hx2), "Draft 2 Mean", pos=3)

text(10, .05, paste("p-value = ", round(ttest$p.value, 4), "or better reported as p < 0.001"))
