...
hist(grades5, main=labels[6], xlab="Score", 
...
text(mean(grades4, na.rm=T), .06, col=colors[4], 
     bquote(paste(bar(x)[4], " = ", .(round(mean(grades4, na.rm=T), 0)))))

text(mean(grades5, na.rm=T), .08, col=colors[5], 
     bquote(paste(bar(x)[5], " = ", .(round(mean(grades5, na.rm=T), 0)))))

text(mean(grades6, na.rm=T), .1, col=colors[6], 
