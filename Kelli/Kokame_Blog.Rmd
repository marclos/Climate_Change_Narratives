...
*For the purpose of clarity, I changed the raw label of "TMAX" to "Maxmimum Temperatures", and the raw label of "DATE" to "Year". 

```{r checking}
plot(TMAX~DATE, import, ylab = "Maxmimum Temperatures (F)", xlab = "Year")
```

When we first import the data, it is not yet in a form that we can use and visualize well. As you can see, there are several values that don't make sense. How could the maximum temperatures be almost -10000 degrees fahrenheit? It's actually not--the value -9999 is used to represent having no data. We can change these values to "NA" to ensure that the large negative values do not skew our data. As you can see, the dates are also in a format that are currently ineffective, and we will need to address this issue as well.  
...
After changing the -9999 values to NA (or no data), we can check to see if our data is now usable:

```{r}
plot(TMAX~DATE, import[import$DATE<19131231,], ty='l', ylab = "Maxmimum Temperature (F)", xlab = "Year") 
```
...
## How can we convert these dates into a form we can use? 

To get the data to organize like we want it to, we first need to convert the date to "string" or character values. After that, we can convert the strings to a data format. 
...
### Let's test out our date-corrected graph: 
```{r}
plot(TMAX~NewDate, import[import$DATE<19130102,], ty='l', xlab = "Year", ylab = "Maximum Temperature (F)", las =1)
```

As you can see, there is still unfortunately a lot of missing temperature data. But we will make use of the data that is available to us. 
...
# Let's use our data to help us to answer some climate-related questions:  
...
#summary(EwaMax.lm)
plot(TMAX~NewDate, Ewa[Ewa$DATE< 19780101,], ty='p', xlab="Year", ylab="Temperature (Fahrenheit)", las =1, main = "Mean Maximum Temperatures Over Time", pch = 20)
abline(coef(EwaMax.lm), col='red')
```

The trend of maxmimum temperatures has a steeper slope of 7e-05 when excluding all of the missing data, indicating a .007 degree increase per century. This increase is also statistically significant, with a p-value less than .001. 

### We see that there is an increasing trend for maximum temperatures. Would we be able to say the same for minimum temperatures?  

```{r}
# Linear Model
EwaMin.lm <- lm(TMIN~NewDate, data=Ewa[Ewa$DATE< 19780101,])
#summary(EwaMin.lm)
plot(TMIN~NewDate, Ewa[Ewa$DATE< 19780101,], ty='p', xlab="Year", ylab="Temperature (Fahrenheit)", las = 1, main = "Mean Minimum Temperatures Over Time", pch = 20)
abline(coef(EwaMin.lm), col='red')
```


As you can see, minimum temperatures also have an upward trend, with a slope of about 6e-05. This translates into a .006 degree increase each century. This increase in minimum temperatures is also statistically significant, with a p-value again less than .001. 
...
## Are there any trends in specifically January maximum temperatures year to year? 
...
plot(TMAX~YEAR, data=MonthlyMean[MonthlyMean$Month=="01",], ty='p', las = 1, pch = 20, ylab = "Temperature (F)", xlab = "Year", main = "Maximum Temperatures in Janurary Over Time") 
...
plot(TMAX~YEAR, data=MonthlyMean[MonthlyMean$Month=="11",], ty='p', pch = 20, las = 1, xlab = "Year", ylab= "Maxmimum Temperature (F)", main = "Maximum Temperatures in November Over Time") 
...
plot(TMIN~YEAR, data=MonthlyMeanTMIN[MonthlyMeanTMIN$Month=="11",], ty='p', pch = 20, xlab = "Year", ylab = "Temperature (F)", main = "Minimum Temperatures in November Over Time", las = 1) 
...
plot(TMIN~YEAR, data=MonthlyMeanTMIN[MonthlyMeanTMIN$Month=="07",], ty='p', xlab = "Year", ylab = "Temperature (F)", main = "Minimum Temperatures in July Over Time", las = 1, pch = 20) 
...
plot(PRCP~YEAR, data=MonthlyMeanPRCP[MonthlyMeanPRCP$Month=="05",], ty='p', pch=20, xlab="Year", ylab="Precipitation (Inch)", main = "Precipitation is May Over Time", las = 1)
