---
title: 'Climate Change in Eastern Texas'
...
Texas's large coast and overreliance on its abundant natural resources render it particularly vulnerable to climate change. The Environmental Protection Agency estimates that the state has warmed .5-1°F in the past century (“What Climate Change Means for Texas”). Although this number may seem underwhelming, even a minimal increase in temperature can exacerbate issues such as rising sea levels, extreme weather events, drought, and floods.

Alvin, a small city in the Houston metropolitan area of Texas, is home to a weather station that has collected local weather data since 1898, longer than any other weather station in Eastern Texas. The data, available online through the National Oceanic and Atmospheric Administration (NOAA), is helpful in analyzing local weather and climate trends in nearby Houston and Galveston. 
...
To determine whether temperature has increased in Texas over the past century, we will analyze the NOAA data and perform a series of statistical tests to determine the data's validity. 

Let us begin by creating hypotheses. In accordance with the scientific method, the null hypothesis predicts that there is no observed relationship between variables. In our case, this would mean that Texas is not warming. Generally, scientists hope to disprove the null hypothesis and, consequently, accept the alternative hypothesis.


Alternative Hypothesis: The maximum temperature in Alvin, TX has increased from 1898-2013.

Null Hypothesis: The maximum temperature in Alvin, TX has not increased from 1898-2013. 


After inputting the data into R, the program will derive a p-value for us. P-value is used to calculate the probability of an event happening by chance; the smaller the p-value, the lower the probability of that event happening by chance. A p-value of <.05 leads to a rejection of the null hypothesis.
...
Below are the maximum mean temperatures in Alvin, TX from 1898-2013. There is an overall increasing trend in the data. However, with p=0.30, we cannot reject nor accept our null hypothesis because the data is not statistically significant.
...
As shown in the previous graph, there is a huge portion of missing data from about 1920-1960. A gap in data this large poses questions about the validity of the data set. To see if the trend exists independently of this missing data, we can look only at the data collected after 1960. 
...
#### Analyzing individual months

To analyze statistically significant data, we can look at maximum temperatures for each month rather than the entire year. 

In the month of December, for instance, the maximum monthly temperatures from 1898-2013 show an increasing trend (p=.02). We can reject the null hypothesis.
...
The months of October (p=.004) and March (p=.047) from 1898-2013 show similar trends of increasing maximum temperature and are statistically significant, thereby rejecting the null hypothesis. 
```{r, echo=FALSE, include=FALSE}
