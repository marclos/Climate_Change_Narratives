...
Olivia has two sites in her data, so, we need to subset it.
...
Now, the change is `r coef(July.lm)[2]` degress/year or `r round(coef(July.lm)[2]*100, 3)` degress/100 years (YIKES!) with a probability of `r round(summary(July.lm)$coefficients[2,4],4)`. Although we can't reject the null hypothesis, it is worrying!
...
plot(MonthlyMean$TMAX[MonthlyMean$Month=="01"], ty='l')
plot(TMAX~YEAR, data=MonthlyMean[MonthlyMean$Month=="01",], ty='l') 
Feb.lm <- lm(TMAX~YEAR, data=MonthlyMean[MonthlyMean$Month=="01",])
summary(Jan.lm)

abline(coef(Jan.lm), col="red")

```
Now, the change is `r coef(Jan.lm)[2]` degress/year or `r round(coef(Jan.lm)[2]*100, 3)` degress/100 years (YIKES!) with a probability of `r round(summary(Jan.lm)$coefficients[2,4],4)`. Although we can't reject the null hypothesis, it is worrying!
...
Feb.lm <- lm(TMIN~YEAR, data=MonthlyMeanTMIN[MonthlyMeanTMIN$Month=="01",])
summary(Jan.lm)
...
3. In this case, in the month of Febuary, we get a slope, `r coef(Jan.lm)[2]` degress/year and a probability of `r round(summary(Jan.lm)$coefficients[2,4],7)` and an r-sqaured of `r round(summary(Jan.lm)$r.squared, 3)`. Cool! As we might expect, the a small amount of the variance is explained by the "Month". Many things predict temerpature, that year is one, is quite problematic.
